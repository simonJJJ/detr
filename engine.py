...
@torch.no_grad()
def evaluate_kitti(model, criterion, postprocessors, data_loader, device, output_dir):
    model.eval()
    criterion.eval()
    ...
    kitti_evaluator = KittiEvaluator(current_classes=["Car", "Pedestrian", "Cyclist"])
...
@torch.no_grad()
def evaluate_voc(model, criterion, postprocessors, data_loader, device, output_dir):
    model.eval()
    criterion.eval()
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        if voc_evaluator is not None:
            voc_evaluator.update(res, targets)

    # gather the stats from all processes
    metric_logger.synchronize_between_processes()
    print("Averaged stats:", metric_logger)
    if voc_evaluator is not None:
        voc_evaluator.synchronize_between_processes()
